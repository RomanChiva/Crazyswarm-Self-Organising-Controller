...
def initial_position_generator(initial, n, dist):
...
def YAML_gen(positions):
    ...
    - channel: 100
      id: {id}
      initialPosition: [{x}, {y}, 0.0]""".format(id=x, x=pos[0], y=pos[1])
...
def PID (cf, target, k, dist):

    
    
    input_x = cf.position()[0]
    input_y = cf.position()[1]

    target_x = target[0]
    target_y = target[1]

    change_x = (target_x-input_x)
    change_y = (target_y-input_y)


    if np.linalg.norm([change_x, change_y]) > dist*0.4:
        cf.cmdVelocityWorld([change_x*k,change_y*k,0],0)
    else:
        cf.cmdVelocityWorld([change_x*3*k,change_y*3*k,0],0)
...
def euclidean_distance_matrix(drones):
    ...
    positions = np.array([drone.cf.position()[:2] for drone in drones])
...
def run(DRONES, timer):

    while True:

        for drone in DRONES:
            drone.action_manager(DRONES)
...
def sensor_input(dist, DRONES):
    ...
    for i, vector in enumerate(dist_mat):
        ...
        if neighborhood == []:
            print('separation is happening :0')
